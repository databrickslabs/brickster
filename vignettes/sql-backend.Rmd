...
2.  Stage `.parquet` files to a Volume directory, then use `INSERT INTO` or `CREATE TABLE AS SELECT` (CTAS) with `READ_FILES` as the data source
...
When `field.types` is supplied, volume-based writes create the declared schema
before inserting the staged data. For example,
`field.types = c(id = "BIGINT", amount = "DECIMAL(12, 2)", code = "VARCHAR(10)")`
sets those column types; unspecified columns use `dbDataType()`, as in standard
writes. Databricks converts incoming values and enforces the declared limits.
Appends insert into the existing schema, including when the data frame's column
order differs. Without overrides, table creation infers types from Parquet.

Creating or replacing an explicit schema and loading its data are separate SQL
statements. If loading fails, the newly created or replaced table can be empty.
