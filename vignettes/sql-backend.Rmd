---
title: "`{DBI}`/`{dbplyr}` backend"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{`{DBI}`/`{dbplyr}` backend}
...
+=========================================================================================+===============================================================================+======================================================================+
| ODBC                                                                                    | Works with various authentication mechanisms, robust, acceptable performance. | Heavy install that can be difficult to install on corporate devices. |
|                                                                                         |                                                                               |                                                                      |
|                                                                                         |                                                                               | Very slow with larger write operations.                              |
+-----------------------------------------------------------------------------------------+-------------------------------------------------------------------------------+----------------------------------------------------------------------+
| JDBC                                                                                    | Not sure to be honest.                                                        | Slower than ODBC. Requires Java.                                     |
|                                                                                         |                                                                               |                                                                      |
|                                                                                         |                                                                               | Very slow with larger write operations.                              |
+-----------------------------------------------------------------------------------------+-------------------------------------------------------------------------------+----------------------------------------------------------------------+
...
With `disposition = "INLINE"`, reads follow result chunks through the Databricks
API and decode values using the response schema. SQL nulls become typed missing
values. BIGINT and DECIMAL columns remain character to preserve their exact values;
binary columns are lists of raw vectors, with `NULL` for SQL null and `raw(0)` for
empty data. Timestamps use UTC, while timestamps without time zones remain strings.
See `db_sql_query()` for the full conversion rules.

With `disposition = "EXTERNAL_LINKS"`, chunks download to temporary files in
batches controlled by `max_active_connections`. Row limits skip later chunks,
and temporary files are removed after each batch. The returned data still needs
memory proportional to its size. `collect(n = ...)` also passes its limit to
query execution; truncation warnings use a one-row probe.

For incremental reads, call `dbSendQuery()`, then repeat `dbFetch(result, n = ...)`
until `dbHasCompleted(result)` is true. Each successful fetch advances the cursor;
`dbGetRowCount(result)` reports the cumulative rows returned. Copies of a result
share that cursor. Always call `dbClearResult(result)` when finished. Clearing a
result or disconnecting its connection invalidates all copies and requests
cancellation of any statements not known to have finished.

`dbSendStatement()` waits for a write to finish before returning. Its result is
already complete; use `dbGetRowsAffected()` for the affected-row count, then
`dbClearResult()`. These writes retain unlimited polling.
...
`{bricksters}` core dependencies are all [`{WebR}`](https://docs.r-wasm.org/webr/latest/) compatible. The backend uses `{nanoarrow}` as a fallback when `{arrow}` is unavailable (currently `{arrow}` is a `Suggests`).
