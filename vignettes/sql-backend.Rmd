...
Volume-based creation and replacement apply `field.types` through SQL casts.
For example, `field.types = c(id = "BIGINT", amount = "DECIMAL(12, 2)")` casts those
columns and retains Parquet types for the others. Databricks validates the cast
types. For `CHAR`/`VARCHAR` length limits, define the schema with `dbCreateTable()`
and load it with `dbAppendTable()`. Appends use the existing table schema.
